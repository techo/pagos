...
class Payment < ActiveRecord::Base
  ...
  def short_description
    return "#{self.type} - #{self.volunteer.full_name}" if self.type == "EFECTIVO" || self.type == "VISITA"
    return "#{self.type}: #{self.deposit_number} - #{volunteer.full_name}" if self.type == "COMPROBANTE"
  end
end
